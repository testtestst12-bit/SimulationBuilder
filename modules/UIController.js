...
import { stateManager, TokenMode, StateScope } from './StateManager.js';
...
const UI_IDS = Object.freeze({
    ...
    PRESET_SELECT: 'simbuilder-preset-select',
    SCOPE_SELECT: 'simbuilder-scope-select'
});
...
export class UIController {
    ...
    _showSettingsPanel() {
        ...
        const presets = stateManager.presetManager.getAll();

        // Build preset options HTML
        const presetOptionsHtml = presets.length > 0 
            ? presets.map(p => `<option value="${p.id}" ${settings.activePresetId === p.id ? 'selected' : ''}>${this._escapeHtml(p.name)}</option>`).join('')
            : '';
        ...
        panel.innerHTML = `
            ...
            <div class="simbuilder-settings-content">
                ...
                <div class="simbuilder-form-group">
                    ...
                    <select id="${UI_IDS.SCOPE_SELECT}" class="simbuilder-select">
                        <option value="per_chat" ${settings.stateScope === 'per_chat' ? 'selected' : ''}>Per Chat (Separate)</option>
                        <option value="global" ${settings.stateScope === 'global' ? 'selected' : ''}>Global (Shared)</option>
                    </select>
                    <small class="simbuilder-help-text">Global: Same stats in all chats. Per Chat: Different stats per chat.</small>
                </div>

                <hr class="simbuilder-divider" />

                <div class="simbuilder-form-group">
                    <label>Stat Presets</label>
                    <div class="simbuilder-preset-row">
                        <select id="${UI_IDS.PRESET_SELECT}" class="simbuilder-select simbuilder-flex-1">
                            <option value="">-- Select Preset --</option>
                            ${presetOptionsHtml}
                        </select>
                        <button class="simbuilder-btn simbuilder-btn-icon" data-action="apply-preset" title="Apply Preset">✓</button>
                        <button class="simbuilder-btn simbuilder-btn-icon" data-action="edit-preset" title="Edit Preset">✏️</button>
                        <button class="simbuilder-btn simbuilder-btn-icon simbuilder-btn-danger" data-action="delete-preset" title="Delete Preset">🗑️</button>
                    </div>
                </div>

                <div class="simbuilder-form-group">
                    <button class="simbuilder-btn simbuilder-btn-secondary simbuilder-btn-full" data-action="save-preset">
        ...
        panel.querySelector(`#${UI_IDS.SCOPE_SELECT}`).addEventListener('change', (e) => {
            stateManager.setStateScope(e.target.value);
            this._saveSettings();
            this.render();
            this.showNotification(`Scope: ${e.target.value === 'global' ? 'Global' : 'Per Chat'}`, 'info');
        });
            ...
            btn.addEventListener('click', () => {
                switch (btn.dataset.action) {
                    ...
                    case 'reset':
                        ...
                        break;
                    case 'apply-preset':
                        this._applySelectedPreset();
                        break;
                    case 'edit-preset':
                        this._editSelectedPreset();
                        break;
                    case 'delete-preset':
                        this._deleteSelectedPreset();
                        break;
                    case 'save-preset':
    ...
    _applySelectedPreset() {
        const select = document.getElementById(UI_IDS.PRESET_SELECT);
        if (!select || !select.value) {
            this.showNotification('Select a preset first', 'negative');
            return;
        }

        if (stateManager.applyPreset(select.value)) {
            this._saveSettings();
            this._saveState();
            this.render();
            this.showNotification('Preset applied!', 'positive');
        } else {
            this.showNotification('Failed to apply preset', 'negative');
        }
    }

    /**
     * Shows edit preset dialog
     * @private
     */
    _editSelectedPreset() {
        const select = document.getElementById(UI_IDS.PRESET_SELECT);
        ...
        const preset = stateManager.presetManager.get(select.value);
        if (!preset) return;

        this._showPresetDialog(preset, 'Edit Preset', (data) => {
            stateManager.presetManager.update(preset.id, data);
            this._savePresets();
            this._showSettingsPanel(); // Refresh
            this.showNotification('Preset updated!', 'positive');
        });
    }
    ...
    _deleteSelectedPreset() {
        ...
        if (confirm(`Delete preset "${preset.name}"?`)) {
            stateManager.presetManager.remove(select.value);
            if (stateManager.settings.activePresetId === select.value) {
                stateManager.settings.activePresetId = null;
            }
            this._savePresets();
            this._saveSettings();
            this._showSettingsPanel(); // Refresh
    ...
    /**
     * Shows save preset dialog
     * @private
     */
    _showSavePresetDialog() {
        this._showPresetDialog(null, 'Save as Preset', (data) => {
            const preset = stateManager.saveAsPreset(data.name, data.description);
            if (preset) {
                this._savePresets();
                this._showSettingsPanel(); // Refresh
                this.showNotification(`Saved: ${preset.name}`, 'positive');
            }
        });
    }

    /**
     * Shows preset name/description dialog
     * @param {object|null} preset - Existing preset or null for new
     * @param {string} title - Dialog title
     * @param {Function} onSave - Save callback
     * @private
     */
    _showPresetDialog(preset, title, onSave) {
        const defaults = preset || { name: '', description: '' };

        const dialogHtml = `
            <div class="simbuilder-dialog-overlay" id="simbuilder-preset-dialog">
                <div class="simbuilder-dialog">
                    <div class="simbuilder-dialog-header">
                        <h3>${title}</h3>
                        <button class="simbuilder-btn simbuilder-btn-icon" data-action="close">✕</button>
                    ...
                    <div class="simbuilder-dialog-content">
                        <div class="simbuilder-form-group">
                            ...
                            <input type="text" id="preset-name" value="${this._escapeHtml(defaults.name)}" placeholder="e.g., Fantasy RPG, Sci-Fi Stats">
                        </div>
                        <div class="simbuilder-form-group">
                            <label>Description (optional)</label>
                            <textarea id="preset-description" rows="3" placeholder="Describe this preset...">${this._escapeHtml(defaults.description)}</textarea>
                        </div>
                    </div>
                    <div class="simbuilder-dialog-footer">
                        <button class="simbuilder-btn" data-action="cancel">Cancel</button>
        ...
        const dialog = document.getElementById('simbuilder-preset-dialog');
        const closeDialog = () => dialog.remove();
    ...
    _savePresets() {
        ...
        if (context) {
            stateManager.savePresets(context);
        }
    }
    ...
    _saveState() {
        ...
        if (context) {
            ...
            if (stateManager.settings.stateScope === 'global') {
                stateManager.saveGlobalState(context);
            }
        }
    }
